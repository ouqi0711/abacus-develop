...
void energy::cal_bandgap(const elecstate::ElecState* pelec)
{
	...
	for (int ib=0; ib<nbands; ib++)
	{
        ...
        {
            if (!(pelec->ekb(ik,ib) - this->ef > 1e-5) && homo < pelec->ekb(ik,ib)) //1e-5 is applied to avoid numerical issue in NSCF calc
            {
                homo = pelec->ekb(ik,ib);
            }
            if (pelec->ekb(ik,ib) - this->ef > 1e-5  && lumo > pelec->ekb(ik,ib)) //1e-5 is applied to avoid numerical issue in NSCF calc
            {
